...
def read_data_1km(year=2017, doy=300):
  nx=24576
  url=("ftp://sidads.colorado.edu/pub/DATASETS/NOAA/G02156/1km/%s/ims%s%s_1km_v1.3.asc.gz" % (year, year, doy))
  widths=np.full((nx), 1, dtype=int).tolist()
  data=np.array(pd.read_fwf(url, skiprows=30, widths=widths, lineterminator='\n', header=None, compression='gzip'))
  return(data[min_ind_lats:max_ind_lats,min_ind_lons:max_ind_lons])

def read_data_4km(year=2017, doy=300):
  nx=6144
  url=("ftp://sidads.colorado.edu/pub/DATASETS/NOAA/G02156/4km/%s/ims%s%s_4km_v1.3.asc.gz" % (year, year, doy))
  widths=np.full((nx), 1, dtype=int).tolist()
  data=np.array(pd.read_fwf(url, skiprows=30, widths=widths, lineterminator='\n', header=None, compression='gzip'))
  return(data) 
